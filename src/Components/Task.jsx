...
const Todo = ({ classes, toDoObject }) => {
  return (
    <Grid container direction="column">
      <Grid item>
        <Typography className={classes.todoHeading} variant="h6">
          {toDoObject.name}
        </Typography>
      </Grid>
      <Grid item>
        <Typography className={classes.todo} variant="subtitle1">
          {toDoObject.date}
        </Typography>
      </Grid>
    </Grid>
...
const Task = ({
  ...
  const useStyles = makeStyles((theme) => ({
    paper: {
      ...
      position: "relative",
      cursor: "pointer",
      backgroundColor: !toDoObject.completed
        ? "none"
  ...
  return (
    <div>
      <Paper
        onDoubleClick={() =>
          !toDoObject.completed && setReminder(toDoObject.id)
        }
        variant="outlined"
        className={classes.paper}
      >
        <Grid
          className={`${toDoObject.reminder ? classes.before : ""} `}
          container
          alignItems="center"
          justify="flex-start"
        >
          ...
          <Grid className={classes.alignToRight} item lg={4} sm={3} xs={2}>
            <ButtonGroup>
              ...
              {!toDoObject.completed ? (
                <Button
                  onClick={() => completeTodo(toDoObject.id)}
                  size="small"
                  startIcon={<DoneIcon />}
                  variant="outlined"
            ...
            {!toDoObject.completed && (
              <DoneIcon
                onClick={() => completeTodo(toDoObject.id)}
                className={`${classes.icons} ${classes.completeIcon}`}
              />
            )}
            <DeleteIcon
              onClick={() => deleteTodo(toDoObject.id)}
              className={`${classes.icons} ${classes.deleteIcon}`}
            />
          </Grid>
        </Grid>
      </Paper>
